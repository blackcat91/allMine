    ...
    override fun onCreateView(
        ...
        addLinkButton.setOnClickListener(object : View.OnClickListener {
            override fun onClick(v: View?) {
                ...
                val urlInput = credsView.findView<EditText>("url")
                val userInput = credsView.findView<EditText>("username")
                val passInput = credsView.findView<EditText>("password")
                ...
                if (!clipboardText.isNullOrEmpty() && clipboardText.startsWith("http", ignoreCase = true)) {
                    urlInput.setText(clipboardText.toString())
                }
                ...
                AlertDialog.Builder(context ?: throw Exception("Unable to build alert dialog"))
                    ...
                    .setPositiveButton("Save", object : DialogInterface.OnClickListener {
                        override fun onClick(p0: DialogInterface, p1: Int) {
                            ...
                            var url = urlInput.text.trim().toString().replace(Regex("^(HTTPS|HTTP)", RegexOption.IGNORE_CASE)) {
                                it.value.lowercase()
                            }
                            var username = userInput.text.trim().toString()
                            var password = passInput.text.trim().toString()
                            if (name.isNullOrEmpty() || !url.startsWith("http")) {
                                ...
                                try {
                                    ...
                                    if (existingLinks.any { it.name == name }) {
                                        ...
                                        val updatedLinks = existingLinks.toMutableList().apply {
                                            add(Link(name, url, username, password))
                                        }
